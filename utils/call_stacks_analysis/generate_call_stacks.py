...
PARSER.add_argument('-s', '--stack-usage-stat-file')
PARSER.add_argument('-f', '--cflow-output-file')
PARSER.add_argument('-c', '--config-file')
PARSER.add_argument('-d', '--dump', action='store_true', help='Dump debug files')
PARSER.add_argument('-i', '--skip-threshold', type=int, default=0,
...
StackUsage: TypeAlias = dict[str, StackUsageRecord]

Calls: TypeAlias = dict[str, list[str]]
RCalls: TypeAlias = Calls
...
def parse_stack_usage(stack_usage_stat_file: str) -> StackUsage:
        ...
        with open(stack_usage_stat_file, 'r') as file:
                for line in file:
                        ...
                        if found:
                                funcs[found.group(2)] = {'size': int(found.group(1)), 'type': found.group(4)}
                        else:
...
def validate(stack_usage: StackUsage, calls: Calls, config: Config, skip_threshold: int) -> None:
        ...
        for k, v in stack_usage.items():
                ...
                if k in config['dead_end']:
                        continue
                if k in config['whitelist']:
                        continue
                if v['size'] <= skip_threshold:
                        continue
                callers = find_api_callers(k, calls)
                if len(callers) == 0:
